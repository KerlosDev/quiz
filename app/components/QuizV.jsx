...
const QuizV = () => {
    ...
    const quizdata = (email) => {
        GlobalApi.vquiz(email).then(res => {
            setQuiz(res.quizresults || []); // Use default empty array if no quizresults exist
        });
    };
    ...
    if (quiz.length === 0) return <div>Loading...</div>; // Show loading message while data is being fetched
