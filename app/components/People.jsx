...
const People = () => {
    ...
    return (
        <div className='flex justify-center '>
            <div className=' bg-paton gap-4  grid grid-cols-1  lg:grid-cols-2 bg-cover m-5 p-5 rounded-xl '>
                ...
                <div className='backdrop-blur-md p-4  bg-black/20   rounded-xl   '>
                    ...
                    <h5 dir='rtl' className=' flex text-3xl lg:text-4xl justify-center mx-auto  place-items-center gap-2 font-arabicUI3 text-white rounded-xl w-fit  '>عدد المدعوين <IoPersonSharp />
                    </h5>
                    <h3 className=' text-white  text-center mt-5v  text-8xl font-arabicUI3'>
                        {members.length}
                    </h3>

                </div>

                <div className='backdrop-blur-md pt-6 lg:p-4 lg:col-span-2  bg-black/20   rounded-xl   '>

                    <h5 dir='rtl' className=' flex text-sm lg:text-3xl place-items-center gap-2 font-arabicUI3 text-white rounded-xl w-fit mx-auto  '> ايميلات الطلبة اللي دعت صحبها <IoPersonSharp />
                    </h5>
                    <div className=' mx-auto text-white text-center mt-5 w-fit font-arabicUI3'>
                        {Object.entries(emailsByCode).map(([code, count]) => (
                            <div key={code} className='text-lg lg:text-4xl bg-black/20 p-2  m-3 rounded-xl'>
                                {`  ${studentEmails[code] || 'ناس بتهبد كود'} =>   `} <span className=' bg-black/40 px-4 py-2  rounded-full'>{count}</span>
                            </div>
                        ))}
                    </div>
