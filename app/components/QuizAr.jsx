                ...
                <div className='bg-quiz cursor-default bg-cover rounded-xl max-sm:p-4 p-8 max-sm:m-2 m-4'>
                    <div className='backdrop-blur-xl p-3 px-8 rounded-xl outline-dashed outline-white outline-2'>
                        ...
                        <div>
                            ...
                            <div className='mt-10 grid grid-cols-11 max-md:grid-cols-4 max-sm:grid-cols-4 max-lg:grid-cols-5 max-xl:grid-cols-7 gap-3'>
                                {enrolquiz?.question?.map((item, index) => (
                                    <h2
                                        onClick={() => handleClickNumber(index)}
                                        className={`mb-7  cursor-pointer font-arabicUI3 text-2xl p-4 rounded-lg text-center    duration-500 transition active:ring-4 select-none ${activeIndex === index ? "bg-gray-800 text-white scale-125 max-sm:m-0 h-fit mx-4" : "bg-white text-gray-800"}`}
                                        key={index}>
                                        {index + 1}
                                    </h2>
                                ))}
