...
const GreenButton = ({ title, number, handleClick, size = 5, font = "font-arabicUI2" }) => {
    return (

        <div
            onClick={() => handleClick(title, number)}
            className={` hover:scale-110 m-4 ${font} h-40 place-items-center justify-center bg-opacity-50 text-white text-2xl sm:text-3xl lg:text-4xl font-bold transition duration-300 bg-non2 bg-cover gap-2 py-5 rounded-xl cursor-pointer `}
        >
            ...
            <span className={`text-${size}xl `}>{title}</span>
        </div>
    )
}
