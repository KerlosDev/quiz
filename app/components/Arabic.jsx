...
const Arabic = () => {
    ...
    const renderQuizzes = () => {
        let filterKey = '';
        if (numbook === 1) filterKey = 'kayan';
        if (numbook === 2) filterKey = 'emthan';
        if (numbook === 3) filterKey = 'elBayan';
        if (numbook === 4) filterKey = 'alEbda3';
    ...
    return (
        <div className='mx-4 sm:mx-8 lg:mx-20 grid grid-cols-1 lg:grid-cols-4 gap-6'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 lg:col-span-2'>
                <div
                    onClick={() => handleClick('كتاب كيان', 1)}
                    className='hover:scale-110 transition cursor-pointer'>
                    <Image src='/kayan.png' width={400} height={300} alt='chem' className='w-full h-auto' />
                </div>
                <div
                    onClick={() => handleClick('كتاب الامتحان', 2)}
                    className='hover:scale-110 transition cursor-pointer'>
                    <Image src='/emthan.png' width={400} height={300} alt='chem' className='w-full h-auto' />
                </div>
                <div
                    onClick={() => handleClick('كتاب بيان', 3)}
                    className='hover:scale-110 transition cursor-pointer'>
                    <Image src='/bayan.png' width={400} height={300} alt='chem' className='w-full h-auto' />
                </div>
                <div
                    onClick={() => handleClick('كتاب الابداع', 4)}
                    className='hover:scale-110 transition cursor-pointer'>
                    <Image src='/ebda.png' width={400} height={300} alt='chem' className='w-full h-auto' />
                </div>
            </div>
        </div>
    );
