        ...
        <>
            ...
            <div className='bg-white rounded-xl md:p-1 xl:p-1'>
                ...
                {peopleInvented.length > 0 ? (
                    <ul className='font-arabicUI3 xl:text-2xl  bg-black/10   text-sm  lg:text-lg m-4 p-1 md:p-2 border-4 text-center rounded-xl border-black'>
                        {peopleInvented.map((person, index) => (
                            <li className='my-1 xl:my-3' key={index}>{index + 1} - {person.email}</li>
                        ))}
                    </ul>
                ) : (
